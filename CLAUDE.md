...
- **`staff.html`** — Staff Comp page. Multi-row tool: add multiple items with individual units and tiers, apply a comp type multiplier, get per-row breakdown and grand total.
...
- `index.html` → `https://simonstelt.github.io/GTA-Run-Calculator/`
- `staff.html` → `https://simonstelt.github.io/GTA-Run-Calculator/staff.html`
...
- `compMults` — `{ p2p: 1.0, 'server-proc': 0.70, 'server-raw': 0.33 }`
...
compValue = Math.round(total × compMults[compType])
```
